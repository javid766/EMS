...
class EntitlementType extends Model
{
	...
	public function getEntitlementTypes($request, $id, $type) {
		...
		$logData = array('LogName'=>"Entitlement", "ErrorMsg"=>"CALL sp_att_setup_entitlement_type_get($id,$userid,$companyid,$locationid)");
	...
	public function deleteEntitlementType($id, $type) {
		...
		if ($id > 0) {
			...
			/* Logs for stored procedure starts */
			$logData = array('LogName'=>"Entitlement", "ErrorMsg"=>"SET @id = $id; CALL sp_att_setup_entitlement_type_insertupdate(@id, 0, 0, 0, 0, 0, 0, 0, 0, '$action_type')");
	...
	public function createUpdate($request, $id, $sp_type, $type) {
		...
		/* Logs for stored procedure starts */
		$logData = array('LogName'=>"Entitlement", "ErrorMsg"=>"$set_id CALL sp_att_setup_entitlement_type_insertupdate(@id, '$request->vcode', '$vname', $companyid, $is_active, $insertedBy, '$insertedIp', $updatedBy, '$updatedIp', '$sp_type')");
