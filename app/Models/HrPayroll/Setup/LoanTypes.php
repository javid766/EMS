...
{
    use HasFactory;
    protected $table = 'att_setup_loan_types';
	public $timestamps = false;

	public $utilsModel;

	public $PAGE_LINK = '/setup/loan-types';
	...
	public function getLoanTypes($request, $id, $type) {
	...
	public function createLoanTypes($request, $type) {
		...
		if ($id > 0) {
			...
			return $this->utilsModel->returnResponseStatusMessageExtra('success', 'Loan Types created successfully', 'id', $id, $type, $this->PAGE_LINK);
		} else {

			return $this->utilsModel->returnResponseStatusMessage('error', 'There is an error creating Loan Types.', $type, $this->PAGE_LINK);
		}
	}

	public function updateLoanTypes($request, $id, $type) {
		...
		return $this->utilsModel->returnResponseStatusMessageExtra('success', 'Loan Types updated successfully', 'id', $id, $type, $this->PAGE_LINK);
	}

	public function deleteLoanTypes($id, $type) {
		...
		if ($id > 0) {
			...
			return $this->utilsModel->returnResponseStatusMessage('error', "Loan Types id is required", $type, $this->PAGE_LINK);
		}
	}
