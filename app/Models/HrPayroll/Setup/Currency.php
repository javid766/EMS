...
class Currency extends Model
{
	...
	public function getCurrencies($request, $id, $type) {
		...
		$logData = array('LogName'=>"Currency", "ErrorMsg"=>"CALL sp_setup_currency_get($id,$userid,$companyid,$locationid)");
	...
	public function deleteCurrency($id, $type) {
		...
		if ($id > 0) {
			...
			/* Logs for stored procedure starts */
			$logData = array('LogName'=>"Currency", "ErrorMsg"=>"SET @id = $id; CALL sp_setup_currency_insertupdate(@id, 0, 0, 0, 0, 0, 0, 0, '$action_type')");
	...
	public function createUpdate($request, $id, $sp_type, $type) {
		...
		/* Logs for stored procedure starts */
		$logData = array('LogName'=>"Currency", "ErrorMsg"=>"$set_id CALL sp_setup_currency_insertupdate(@id, '$request->vcode', '$vname', $is_active, $insertedBy, '$insertedIp', $updatedBy, '$updatedIp', '$sp_type')");
