...
{
    use HasFactory;
    protected $table = 'att_setup_grade';
	public $timestamps = false;

	public $utilsModel;

	public $PAGE_LINK = '/setup/grade';
	...
	public function getGrades($request, $id, $type) {
	...
	public function createGrade($request, $type) {
		...
		if ($id > 0) {
			...
			return $this->utilsModel->returnResponseStatusMessageExtra('success', 'Grade created successfully', 'id', $id, $type, $this->PAGE_LINK);
		} else {

			return $this->utilsModel->returnResponseStatusMessage('error', 'There is an error creating grade.', $type, $this->PAGE_LINK);
		}
	}

	public function updateGrade($request, $id, $type) {
		...
		return $this->utilsModel->returnResponseStatusMessageExtra('success', 'Grade updated successfully', 'id', $id, $type, $this->PAGE_LINK);
	}
	...
	public function deleteGrade($id, $type) {
		...
		if ($id > 0) {
			...
			return $this->utilsModel->returnResponseStatusMessage('success', 'Grade deleted successfully', $type, $this->PAGE_LINK);

		} else {

			return $this->utilsModel->returnResponseStatusMessage('error', "Grade id is required", $type, $this->PAGE_LINK);
		}
	}
