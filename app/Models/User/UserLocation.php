...
class UserLocation extends Model
{
	...
	public $PAGE_LINK = '/user/location/userlocation';
	...
	public function getUserLocations($request, $id, $type) {
		...
		$userLocations = UserCompany::hydrate(
			DB::select('CALL sp_user_location_get('. $id .', '. $userid .', '. $companyid .', '. $locationid .')')
		);
	...
	public function createUserLocation($request, $type) {
		...
		if ($id > 0) {
			...
			return $this->utilsModel->returnResponseStatusMessageExtra('success', 'User location created successfully', 'id', $id, $type, $this->PAGE_LINK);
		} else {

			return $this->utilsModel->returnResponseStatusMessage('error', 'There is an error creating user location.', $type, $this->PAGE_LINK);
		}
	}

	public function updateUserLocation($request, $id, $type) {
		...
		return $this->utilsModel->returnResponseStatusMessageExtra('success', 'User location updated successfully', 'id', $id, $type, $this->PAGE_LINK);
	}

	public function deleteUserLocation($id, $type) {

		if ($id > 0) {

			$result = DB::select('CALL sp_user_location_insertupdate(
				?, 0, 0, 0, 0, 0, 0, 0, 0, 0,
			...
			return $this->utilsModel->returnResponseStatusMessage('success', 'User location deleted successfully', $type, $this->PAGE_LINK);

		} else {

			return $this->utilsModel->returnResponseStatusMessage('error', "User location id is required", $type, $this->PAGE_LINK);
		}
	}
	...
	public function createUpdate($request, $id, $sp_type, $type) {
		...
		return DB::select('CALL sp_user_location_insertupdate(
			?,
			"'. $request->vname .'",
			'. $request->userid .',
			'. $request->locationid .',
