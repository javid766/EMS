...
class RosterShiftController extends Controller
{
    ...
    public function index(){
        ...
        return view('hrpayroll.setup.roster_shift');
    }

    public function getData($request, $id=0){

        return $this->attRosterShiftModel->getRosterShifts($request, $id, $this->utilsModel->CALL_TYPE_DEFAULT);
    ...
    public function fillForm(Request $request, $id){
        ...
        if($modelData){
            ...
            return response(array(
                'error' => 1,
                'data' => 'RosterShift Shifts doesn\'t exists!!',
            ));
        }
    ...
    public function save(Request $request){
        ...
        if(ctype_alpha($vcode)==false || strlen($vcode)>1){
        
            return redirect()->back()->withInput($request->input())->with('error', 'Code Contains Only Single Character Alphabet.');
        }
        
        if($request->vcode=="o" || $request->vcode=="O" || $request->vcode=="z" || $request->vcode=="Z" ){
        
            return redirect()->back()->withInput($request->input())->with('error', 'Cannot Entre code o or O.');
        }
        ...
        $roasterShift = RosterShift::where('id', '<>', $request->id)->where('vcode', trim($request->vcode))->orwhere('vname', trim($request->vname))->where('id', '<>', $request->id)->first();
        ...
        if($request->id){       
            $id = $request->id;
            return $this->attRosterShiftModel->updateRosterShift($request, $id, $this->utilsModel->CALL_TYPE_DEFAULT);
        
        } else {

            return $this->attRosterShiftModel->createRosterShift($request, $this->utilsModel->CALL_TYPE_DEFAULT);
        }
    }

    public function delete($id){

        return $this->attRosterShiftModel->deleteRosterShift($id, $this->utilsModel->CALL_TYPE_DEFAULT);
    }
