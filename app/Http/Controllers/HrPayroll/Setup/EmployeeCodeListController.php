...
class EmployeeCodeListController extends Controller
{
    ...
    public function index(Request $request){
        ...
        return view('hrpayroll.setup.employeecodelist', compact('allLocations'));
    }
    ...
    public function save(Request $request){
        ...
        if($request->id){
            ...
            $probationStatus = ProbationStatus::where('vname', trim($request->vname))->where('id', '<>', $request->id)->first();

        } else {

            $probationStatus = ProbationStatus::where('vname', trim($request->vname))->first();
        }
    ...
    public function getLocations($request){
        ...
        $id=0;
        return  $this->locationModel->getLocations($request, $id, $this->utilsModel->CALL_TYPE_DEFAULT)->pluck('vname', 'id');
    }
}
