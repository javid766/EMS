...
class ProjectController extends Controller
{
	...
	public function index(){
		...
		return view('hrpayroll.setup.project');
	}

	public function getData($request, $id=0){

		return $this->projectModel->getProjects($request, $id, $this->utilsModel->CALL_TYPE_DEFAULT);	 
	}
	...
	public function save(Request $request){
		...
		$project = Project::where('id', '<>', $request->id)->where('vcode', trim($request->vcode))->orwhere('vname', trim($request->vname))->where('id', '<>', $request->id)->first();
		...
		if($request->id){
			...
			return $this->projectModel->updateProject($request, $id, $this->utilsModel->CALL_TYPE_DEFAULT);
			
		} else {
			
			return $this->projectModel->createProject($request, $this->utilsModel->CALL_TYPE_DEFAULT);
		}
	}

	public function delete($id){

		return $this->projectModel->deleteProject($id, $this->utilsModel->CALL_TYPE_DEFAULT);
	}
