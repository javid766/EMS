...
(function($) {
	'use strict';
	//Account Financial Year Data Table
	$(document).ready(function()
	{
		var searchable = [];
		var selectable = []; 

		var dTableFinancialYear = $('#accountFinancialYearTable').DataTable({
			...
			{data:'istransactional', name: 'istransactional'},
	...
	$(document).on('click','#deleteRecord' ,function(){
		var id = $('#id').val();	
		window.location.href = 'financial-year/delete/'+id;
	});
	//Edit Button
	$(document).on('click','#editBtn' ,function(){
		...
		$('#id').val(id);
		$.ajax({
			url: 'financial-year/fillform/'+id,
			type: 'get',
			dataType:"JSON",
			...
			success:function(data){
				...
				data['istransactional'] == 1 ? $("#istransactional").prop('checked', true):
				$("#istransactional").prop('checked', false);
